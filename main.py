...
def save():
    data = {
        account_name: [account_password, player_money],
    }
...
load()
player_money = round(loaded_data[account_name][1], 2)
...
def update_player_cards():
    ...
    for direct in player_cards:
        player_cards_images.append([pygame.image.load(f'graphics/cards/{direct[0]}/{direct[1]}.png'), [position, 500]])
        position += 100
...
def start_click():
    ...
    calc_tokens()
    bet_and_start = 1
    player_money -= round(bet_value, 2)
    bet_tokens.clear()
...
while True:
    ...
    if bet_and_start != 0:
        ...
        if game_ended == 1:
            ...
            round_over += 1
            if round_over == 1:
                player_money += round((random.uniform(1.5, 10) * bet_value), 2)
        elif game_ended == -1:
            ...
            if game_ended == 0 and bet_and_start != 0:
                ...
                if bet_button.collidepoint(event.pos):
                    ...
                    for bet_token in bet_tokens:
                        if bet_token[0].get_rect(topleft=(bet_token[1][0], bet_token[1][1])).collidepoint(event.pos):
